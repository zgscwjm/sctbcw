...
<body>
				...
				<div class="row mt">
					<div class="col-md-12">
						<div class="content-panel">
							...
							<c:forEach items="${reviewlist}" var="reviewdata" varStatus="coun">	
									
									<tr>
										<td>${coun.count}</td>
										<td class="hidden-phone">${reviewdata.auditingDepartment }</td>
										<td>${reviewdata.trainapply.id }</td>
										<td>${reviewdata.trainapply.traininfo.projectName }</td>
										<td>
										<c:forEach items="${reviewdata.user.teachstaffinfos }" var="userinfo">
										${userinfo.name}
										</c:forEach>
										</td>
										<td>${reviewdata.auditingOpinion }</td>
										<th>${reviewdata.denyReason }</th>
										<td>
										<c:if test="${reviewdata.auditingOpinion=='待审核' }">
										<button type="button" class="btn btn-theme" id="tg_btn" dataid="${reviewdata.id }" otherval="${reviewdata.trainapply.id }">批准</button>
										<button type="button" class="btn btn-theme04" id="jj_btn" dataid="${reviewdata.id }" otherval="${reviewdata.trainapply.id }">拒绝</button>
										</c:if>
										<c:if test="${reviewdata.auditingOpinion=='通过'||reviewdata.auditingOpinion=='拒绝' }">
										<button type="button" class="btn btn-default" disabled="disabled">已${reviewdata.auditingOpinion }</button>
										</c:if>
											<!-- <button class="btn btn-success btn-xs" type="button" id="see_btn">
												<i class="fa fa-check"></i>
											</button>
    ...
    <script src="<%=path%>/sctbc/js/AuditingOpinion/auditingOpinion.js"></script>
    <script src="<%=path%>/sctbc/js/AuditingOpinion/auditingOpinion_budle.js"></script>
</body>
</html>
